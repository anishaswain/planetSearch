  ...
  return (
    <React.Fragment>
      {isLoading ? (
        ...
        data.map((item) => {
          const color = colors.filter((color) => color.id === item.color)[0][
            "name"
          ];
          const shape = shapes.filter((shape) => shape.id === item.shape)[0][
            "name"
          ];
          ...
          return (
            <Card className="resultCard" key={item.id}>
              <Space direction="vertical">
                ...
                <Text>
                  {item.name} has {color} color and {shape} shape and {size}{" "}
                  size.
                </Text>
              </Space>
            </Card>
